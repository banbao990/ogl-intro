...
PbrMaterial::PbrMaterial(bool show_base_color) {
  ...
  _program =
      Program::create_from_files("shaders/13-mirror/pbr.vert", frag_file);
...
#undef INIT_UNIFORM_LOCATION
  ...
  _clip_plane_location = glGetUniformLocation(_program->get(), "clip_plane_ws");
  _clip_enabled_location =
      glGetUniformLocation(_program->get(), "clip_enabled");
...
#undef ASSIGN_TEXTURE
  ...
  glUniform4fv(_clip_plane_location,
               1,
               reinterpret_cast<const GLfloat *>(&clip_plane_ws));
  glUniform1i(_clip_enabled_location, clip_enabled ? 1 : 0);
...
MirrorMaterial::MirrorMaterial() {
  _program = Program::create_from_files("shaders/13-mirror/mirror.vert",
                                        "shaders/13-mirror/mirror.frag");
  _model_location = glGetUniformLocation(_program->get(), "model");
  _view_location = glGetUniformLocation(_program->get(), "view");
  _projection_location = glGetUniformLocation(_program->get(), "projection");
  _tint_location = glGetUniformLocation(_program->get(), "tint");
  _reflectivity_location =
      glGetUniformLocation(_program->get(), "reflectivity");
  _tint_strength_location =
      glGetUniformLocation(_program->get(), "tint_strength");
}

void MirrorMaterial::use() {
  glUseProgram(_program->get());
  glUniformMatrix4fv(
      _model_location, 1, GL_FALSE, reinterpret_cast<const GLfloat *>(&model));
  glUniformMatrix4fv(
      _view_location, 1, GL_FALSE, reinterpret_cast<const GLfloat *>(&view));
  glUniformMatrix4fv(_projection_location,
                     1,
                     GL_FALSE,
                     reinterpret_cast<const GLfloat *>(&projection));
  glUniform3fv(_tint_location, 1, reinterpret_cast<const GLfloat *>(&tint));
  glUniform1f(_reflectivity_location, reflectivity);
  glUniform1f(_tint_strength_location, tint_strength);
  glDisable(GL_CULL_FACE);
}
