...
OutlineMaterial::OutlineMaterial() {
  _program = Program::create_from_files("shaders/12-stencil/outline.vert",
                                        "shaders/12-stencil/outline.frag");
  ...
  _color_location = glGetUniformLocation(_program->get(), "outline_color");
  _width_location = glGetUniformLocation(_program->get(), "outline_width");
}

void OutlineMaterial::use() {
  glUseProgram(_program->get());
  glUniformMatrix4fv(
      _model_location, 1, GL_FALSE, reinterpret_cast<GLfloat *>(&model));
  glUniformMatrix4fv(
      _view_location, 1, GL_FALSE, reinterpret_cast<GLfloat *>(&view));
  glUniformMatrix4fv(_projection_location,
                     1,
                     GL_FALSE,
                     reinterpret_cast<GLfloat *>(&projection));
  glUniform3fv(_color_location, 1, reinterpret_cast<GLfloat *>(&color));
  glUniform1f(_width_location, width);
  glDisable(GL_CULL_FACE);
}
