...
private:
  void init() override {
    ...
    _outline_material = std::make_unique<OutlineMaterial>();
...
#undef ASSIGN_FIELD
  ...
  void draw_ui() {
    ...
    if (ImGui::CollapsingHeader("Stencil Outline")) {
      ImGui::PushID(id++);
      ImGui::Checkbox("Enabled", &_outline_enabled);
      ImGui::ColorEdit3("Color", &_outline_material->color.x);
      ImGui::SliderFloat("Width", &_outline_material->width, 0.0f, 0.08f);
      ImGui::TextUnformatted("The PBR pass writes 1 to stencil; the outline");
      ImGui::TextUnformatted("pass draws only where stencil is not 1.");
      ImGui::PopID();
    }
  ...
  void draw_scene() {
    ...
    if (_outline_enabled) {
      MICROPROFILE_SCOPEGPUI("Stencil Outline", 0xFF7317);
      MICROPROFILE_SCOPEI("Main", "Stencil Outline", 0xFF7317);

      // Keep only fragments outside the pixels marked by the PBR passes.
      ...
      _outline_material->view = view;
      _outline_material->projection = projection;
      for (auto &draw : _scene->draws) {
        for (auto &prim : _scene->meshes[draw.index]) {
          _outline_material->model = draw.transform;
          _outline_material->use();
          prim.mesh->draw();
        }
      }
    }

    // Do not leak the stencil state into the tone-mapping pass.
    glStencilMask(0xFF);
  ...
  void update() override {
    ...
    _last_time = now;
    // Avoid a large camera jump after a breakpoint or a dragged window.
  ...
  std::unique_ptr<OutlineMaterial> _outline_material{};
  bool _outline_enabled = true;
